...
def download_coco_subset(num_images=1000):
    ...
    out_dir = os.path.join(base_dir, 'train2017_subset')
    
    # יצירת התיקיות הנדרשות
    os.makedirs(base_dir, exist_ok=True)
    os.makedirs(out_dir, exist_ok=True)

    # 1. הורדה וחילוץ של קובץ האנוטציות (אם עדיין לא קיים)
    if not os.path.exists(ann_file):
        ...
        # מחיקת קובץ ה-ZIP כדי לחסוך מקום
        os.remove(ann_zip_path)

    # 2. קריאת האנוטציות והורדת התמונות
    print("Loading annotations...")
    with open(ann_file, 'r') as f:
        coco_data = json.load(f)

    # לוקחים רק את כמות התמונות שביקשנו
