...
#         # Text-Alignment Metric
#         print("Loading CLIP model...")
#         self.clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch16", use_safetensors=True).to(
#             self.device)
#         self.clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch16", use_safetensors=True)
#         self.clip_scores = []
#
...
#         self.dinov2_scores = []
#
#         # Lists to store raw DINOv2 features for PRDC calculation
#         self.prdc_real_features = []
#         self.prdc_gen_features = []
#
#         self.ratio_buckets = {f'{i}-{i + 5}': 0 for i in range(0, 91, 5)}
#         self.bucket_keys_map = {i: f'{i}-{i + 5}' for i in range(0, 91, 5)}
#
...
#             results[self.RECALL] = float(prdc_dict['recall'])
#             results[self.DENSITY] = float(prdc_dict['density'])
#             results[self.COVERAGE] = float(prdc_dict['coverage'])
#         except ImportError:
#             print("WARNING: 'prdc' library not found. Skipping PRDC metrics. Install with: pip install prdc")
...
#             feat2 = self.dinov2(self._prepare_for_dino(img2))
#             sim = F.cosine_similarity(feat1, feat2)
#         return sim.mean(), feat1.cpu().numpy().squeeze(), feat2.cpu().numpy().squeeze()
#
#     @staticmethod
#     def _prepare_for_dino(img_tensor, patch_size: int = 14):
#         if img_tensor.dim() == 3:
