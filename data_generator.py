...
def main(args):
    ...
    # create instance of the pipeline class
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sa_resize_mode", default='nearest', type=str,
                        help="interpolation mode for self attention")
    parser.add_argument("--ca_resize_mode", default='nearest', type=str,
                        help="interpolation mode for cross attention")
    parser.add_argument("--ignore_cross_attention", action='store_true',
                        help="ignore cross attention upgrades")

    # Pre-Processing mask dilation, feathering, and negative prompt
    parser.add_argument("--pp_feather_radius", default=5, type=int,
                        help="feathering kernel size for preprocessing")
    parser.add_argument("--pp_dilate_kernel_size", default=3, type=int,
                        help="dilation kernel size for preprocessing")
    parser.add_argument("--use_negative_prompt", action='store_true',
                        help="dilation kernel size for preprocessing")
    parser.add_argument("--ignore_improvement_v5", action='store_true',
                        help="remove effect of V5 (mask preprocessing and negative prompt)")

    # Soft Masking in Cross-Attention
    parser.add_argument("--sm_dilation_kernel", default=3, type=int,
                        help="dilation kernel for soft masking")
    ...
    parser.add_argument("--use_dynamic_schedule", action='store_true',
                        help="use dynamic schedule for resampling")
    ...
    parser.add_argument("--dmb_blur_kernel_size", default=5, type=int,
                        help="dynamic mask blending blur kernel size")
    parser.add_argument("--dmb_sigma", default=5.0, type=float, help="dynamic mask blending sigma")

    # Organic masking using Perlin noise
    parser.add_argument("--om_noise_res", default=4, type=int,
